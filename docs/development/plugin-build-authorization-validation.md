...
- **S3 服务核验（ADR 0005 D21；政策已由 #112 supersede）**：构建授权与 `hdsl.services.provides` 核验是**正交轴**。授权只在“是否允许执行安装期脚本”上生效，不得被当作对服务轴的可信声明，也不得把未知服务轴变为已核验/已确认。通过构建授权安装的新来源，卸载**不再**因服务轴 unknown 而阻塞：核验状态只作 `riskItems` 信息项，不得再写“无法验证服务依赖，暂不能卸载”的阻断文案。
...
| opt-in 真实 desktop（production Electron + bridge） | 真实 UI 预览/未授权拒绝/勾选授权应用 4 marker/无沙箱文案/授权配置不持久化/漂移旧授权 API 级拒绝 | **PASS**（QA 子任务，代码 `e80fe55` + fixture `base-v3 71f9a972…`；未注入 env；未跑 start）。marker 边界：preview 未单独采样（仅“经 preview+未授权 apply 后 0 marker”）；漂移无受控前后对照，“执行前拒绝”由 API 守卫证据支持 |
