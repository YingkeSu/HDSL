...
结论先行：**这是实验性桌面构建。启动未验证；DSH 环境创建/安装当前被平台契约与目录限制挡住，且本仓没有 Windows 主机证据。** 不要把它描述为“Windows 版本已可用”。
...
该工作流是**受控、仅构建**的：`permissions: contents: read`，不含签名/发布凭据，不创建 Release，保留 14 天 artifact；触发为 `workflow_dispatch` 与打包相关路径的 `push`。
...
因此当前的准确表述是：**DSH 创建/安装既不是“已被平台契约拒绝”，也不是“已被支持”**——它会在错误的平台上推进到产物阶段再失败；win32 凭据解析则是 fail-closed 的。修复需要单独变更与独立验证（不能为出包放宽任何安全门），并单独跟踪。
