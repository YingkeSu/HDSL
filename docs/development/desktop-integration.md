...
- 无效的显式覆盖（悬空 flag、空/空白值、下一 token 是 switch、`HDSL_DATA_ROOT` 已设置但为空）会在任何 `userData` 访问之前以固定 stderr 信号 `[hdsl] launch args invalid kind=… reason=…` 拒绝启动（退出码 2），绝不回退默认 profile；未设置的 `HDSL_DATA_ROOT` 仍是正常默认启动。重复 flag 为文档化的 first-wins，但任一无效出现不会被后续有效值救回。
- 旧 profile 迁移的「是否在用」探测按平台：POSIX 读 Chromium `SingletonLock` 的 `<hostname>-<pid>` 并判进程存活；Windows 无该文件，改看 `<legacy>/lockfile`（Chromium 以 `FILE_FLAG_DELETE_ON_CLOSE` 持有）——存在或不可读时保留 legacy、不 rename，与 POSIX 守卫对称并避免依赖 OS 拒绝。Windows 实机未验证。
...
- 诊断导出：用测试入口 `--hdsl-qa-export-path <file>`，通过 `window.hdsl.call({apiVersion:'1.2',
