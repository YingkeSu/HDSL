...
| M3 整合包 | 004 Schema（packs）、导入导出、精确重建 | M2 | 无凭据泄漏、拒绝恶意包、重建成功 |
...
2026-09-24 核对基线 `97c19f9`：T001–T007 与 T008a 已合入（T008b Windows 未测）；T006/T007/T008a 的真实桌面验收证据见 [validation-001.md](validation-001.md)。macOS 插件 MVP #73 及 #74–#79 已关闭；#88 退出观察与 #95 GitHub 403 分类已修复；契约 1.1 已冻结（`contracts-v1.1.0`），1.2（`entries.patch` / E1-T1 #135）已实现、标签待创建。M2 [#9](https://github.com/YingkeSu/HDSL/issues/9) 的可实现子切片已合入，剩余仅 E1 运行期 ACTIVE 确认（no-go，deferred）与 B1 npm registry 来源信任。整合包（#101/#102）与内测分发包（#103）仍待实现。既有子集验收不代表任意第三方插件可无损卸载或全部运行期生效等价性已证。
