...
- **清理目标唯一性（非归属证明）**：清理 / 归属匹配要加记录唯一标识并让每个 harness 使用独立夹具副本，避免并行测试跨文件广播误杀；这只是目标唯一性 / 防碰撞（必要但**不充分**），路径 / command fragment / generation 双匹配仍不可证明归属，归属以「归属证据」为准，不可证即受控失败（证据：https://github.com/YingkeSu/HDSL/issues/57）。
