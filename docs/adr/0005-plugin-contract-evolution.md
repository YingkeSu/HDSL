...
- **"会被破坏"的可判定性分栏（诚实收窄 AC 口径，S3 实现）**：rc.2 `cordis.patch.yml` 的根是**序列**，条目为 `- insert: [ {id, name, …} ]`（`name` = 包引用，`id` = profile 内的行身份）与 `- id: X` 行覆盖；`inject` 是 **Cordis 服务名**，不是包引用。
  - **可静态判定并拦截**：(i) 其它源 insert 的行 `name` == 被移除包；(ii) 其它源以 `- id: X` 行覆盖命中被移除插件 insert 的 `rowIds`（配置级破坏）。
  - **不可静态判定（不拦截，风险声明 + 重启验收）**：某插件**提供**的 Cordis 服务被其它层 `inject` —— 服务名到包名需要插件代码/元数据，patch 不含该映射；把 `inject` 当包引用会假阳性并永久阻塞卸载，故不采用。预览 `riskItems` 必须写明该限制（"无静态引用 ≠ 无影响"），UI 不得把"未发现引用"呈现为"安全"。
  - 可选透明项（**永不阻塞**）：被移除插件行中出现的服务名与其它层 `inject` 服务名重叠 → 作为 `riskItems` 警告列出。
- **卸载验收必含**（非可选）：同 fixture **安装 → 卸载 → 重启**，以「活动代际记录 + 受管 DSH 离线解析组合树」判定启用集合不再包含该包；若配置解析被破坏，必须表现为**可见的受控失败**，不得静默通过。
