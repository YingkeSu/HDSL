  ...
  buildTarGz([
    ...
    },
    {
      ...
      content: [
        ...
        "if (args.includes('--from-default-profile') && profileIndex >= 0) {",
        '  const name = args[profileIndex + 1];',
        '  const home = process.env.DSH_HOME;',
        "  const dir = join(home, 'profiles', name);",
        '  mkdirSync(dir, { recursive: true });',
        "  writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'dsh-profile-fixture', dsh: { profile: { bundles: ['@deepseek-ai/dsh-base'] } } }));",
        "  writeFileSync(join(dir, 'cordis.patch.yml'), '# fixture patch\\n');",
        '}',
        `console.log('${version}');`,
        '',
      ].join('\n'),
